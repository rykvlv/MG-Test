		...
		public bool ServeOrder(Order order) {
			...
			foreach ( var customerPlace in CustomerPlaces ) {
				if ( customerPlace.CurCustomer != null && 
				     customerPlace.CurCustomer.HasOrder(order) && 
				     customerPlace.CurCustomer.WaitTime < minWaitTime ) {
					curCustomer = customerPlace.CurCustomer;
					minWaitTime = customerPlace.CurCustomer.WaitTime;
				}
			}
			
			if ( curCustomer == null ) return false;

			curCustomer.ServeOrder(order);
			if ( !curCustomer.OrderPlaces.Find(x => x.IsActive) ) FreeCustomer(curCustomer);
